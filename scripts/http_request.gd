...
@onready var chat_node = get_parent()
...
func get_first_question():
	var response = http_request.request("http://127.0.0.1:5000/api/data")
	if response != OK:
		print("Error making GET request:", response)
	else:
		chat_node.display_ai_response(response)
...
### obselete function - for debugging
